...
draw_rectangle(_panel_x + 10, _btn_y, _panel_x + _panel_w - 10, _btn_y + 25, false);
draw_set_color(c_white);
draw_set_halign(fa_center);
draw_text(_panel_x + _panel_w * 0.5, _btn_y + 12, "Charger (Entrée)");

if (mouse_check_button_pressed(mb_left) &&
    point_in_rectangle(mouse_x, mouse_y, _panel_x + 10, _btn_y, _panel_x + _panel_w - 10, _btn_y + 25)) {
    if (selected_idx >= 0 && selected_idx < array_length(available_levels)) {
...
if (keyboard_check_pressed(vk_enter) && selected_idx >= 0 && selected_idx < array_length(available_levels)) {
    load_and_edit(available_levels[selected_idx]);
}
